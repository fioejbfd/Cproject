...
//해당 박식의 구현은 주로, 자기 자신을 다시 호출하는 재귀식 설계로
//구현되는 경우가 많습니다.
...
void Backtracking(int N, int M, int depth, int path[])
{
	...
	for (int i = 1; i <= N; i++)
	{
		...
		if (!used[i])
		{
			used[i] = 1; //사용중이라고 설정하겠습니다.(프로그래밍에서 1은 true, 0은 false를 의미합니다.)
			path[depth] = i; //현재의 i 값을 작업 순서(depth)에 있는 path에 등록합니다.

			//7. 백트래킹 작업(다음 단계로 넘어가기)
			//depth(진행순서)의 값을 1 증가시켜서, 다시 함수(Backtracking)를 호출하는 작업을 진행합니다.
			Backtracking(N, M, depth + 1, path);

			//8. 위의 작업이 마무리되면 사용했넌 used[i]를 다시 0으로 만들어서 비사용 처리 진행
			used[i] = 0;
		}
	}
}
